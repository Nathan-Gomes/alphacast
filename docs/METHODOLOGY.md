...
- **Interval:** a 95% interval for mean Rank IC from a circular block bootstrap (2,000 resamples of three-month blocks, fixed seed). Blocks keep neighbouring months together; wrapping around the end of the sample stops the first and last months being under-drawn, which in a plain moving-block bootstrap shifted the ensemble's interval up by about 0.005. Random Forest's interval is 0.005 to 0.045; every other model's includes zero. Like the unadjusted p-value, it describes one model on its own.
- **Signal decay:** each month's scores are also scored against 5-, 10-, 40- and 60-session sector-relative outcomes. Folds are about 21 sessions apart, so 40- and 60-session outcomes overlap the next one or two folds' outcomes; their t-statistics use Newey-West standard errors with that many lags, which the plain formula would overstate. Random Forest's IC peaks at 10 sessions (0.038, t = 3.6) and holds at 60 (0.038, t = 2.7), so the signal is not confined to the horizon it was trained on.
...
- **Volatility neutralisation:** optionally, each month's scores are regressed on the stocks' 60-session volatility rank and only the residual is ranked, so the sleeve cannot earn its return by holding the riskiest names. For Random Forest the Rank IC is unchanged (0.024) while beta falls from 1.30 to 1.16 and alpha from about 6% to 4% a year.
- **Book size:** each model's fold scores are rebuilt as sleeves of 5, 10, 15, 20 and 30 names (up to half the universe) with the same cadence, costs, cap and a proportionally scaled buffer. Nothing is refitted. For Random Forest net Sharpe stays between 1.15 and 1.28 at every size, above the universe's 1.09, and active return rises from 3.6% a year at 30 names to 21.5% at 5, which is the pattern an informative ranking should produce. Ridge and Elastic Net only reach the universe's Sharpe with 20 or more names.
